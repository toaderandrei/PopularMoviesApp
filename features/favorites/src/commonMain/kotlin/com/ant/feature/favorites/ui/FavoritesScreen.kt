...
import com.ant.resources.empty_movie_results
import com.ant.resources.empty_results
    ...
    Column(modifier = modifier.fillMaxSize()) {
        TabRow(selectedTabIndex = uiState.selectedTab.ordinal) {
            ...
            Tab(
                ...
                text = { Text(stringResource(Res.string.tvshow)) }
            )
        }
                    ...
                    when (uiState.selectedTab) {
                        FavoriteTab.MOVIES -> {
                            if (uiState.favoriteMovies.isEmpty()) {
                                EmptyState(
                                    message = stringResource(Res.string.empty_movie_results),
                                    modifier = Modifier.fillMaxSize()
                                )
                            } else {
                                LazyVerticalGrid(
                        ...
                        FavoriteTab.TV_SHOWS -> {
                            if (uiState.favoriteTvShows.isEmpty()) {
                                EmptyState(
                                    message = stringResource(Res.string.empty_results),
                                    modifier = Modifier.fillMaxSize()
                                )
                            } else {
                                LazyVerticalGrid(
